...
- **Where the lock box lives (July 2026):** the player's usable lock box is the `is-pclocks` strip inside their own info-strip score card (`#is-locks-1` / `#is-locks-2`). **Both** cards render real bordered lock chips at all times — the acting player's are clickable, the other player's are inert (`.lock-inv-btn--static`: disabled, `tabindex=-1`, no listener, full opacity). They deliberately look identical: a card must not lose its lock frames just because the turn passed to the other side. (The non-acting card previously degraded to a plain `🔒3 🔒3 🔒5` text summary via the now-removed `lockSummaryText`.) The original `#lock-inv-display` picker sits in `.right-panel`, which is `display:none !important` (the side-panel layout was retired), so rendering only there left nothing on screen to click. Both are filled by `renderLockInventory` via the shared `fillLockBox` helper. **Stub-DOM unit tests cannot catch a regression here** (no CSS/layout) — `tests/e2e/lock-box.spec.js` guards it in a real browser.
- **Lock cost feedback (July 2026):** the 10-point charge is previewed, then animated. While a lock is pending on the board, a red `−10` price tag renders on the lock cell (`.spine-lock-cost`) and on the owner's score card (`#is-cost-1`/`#is-cost-2`, `.is-visible`, via `renderPendingLockCost`). **This is view-layer only — `state.scores` is NOT charged until the move commits**, because a pending lock can still be moved, returned to the box, or lost to a rejected word (the engine never burns a lock on rejection), and online scores only change through `commitTransaction`. On commit, `detectCommittedLock` (an id-diff in `renderAll`, seeded at mount so resumed games don't animate) triggers `playLockCostFlight`: a `−10` chip flies from the lock cell to `scoreTargetForSlot(ownerSlot)` over `LOCK_COST_FLIGHT_MS` (520ms), and `renderScores` holds the count-down that long so number and chip land together. All three elements set `direction: ltr` — `−10` renders as `10−` in the RTL layout otherwise.
- **Lock placement (July 2026) mirrors tile placement — no auto-placement.** The player clicks a lock in the lock box → that lock glows (`.lock-inv-btn.active`, index-based so a box holding duplicate durations lights only the one clicked). They then click an empty on-grid cell → the lock is previewed there (`.spine-pending-lock-cell`) and **disappears from the box** (`displayLockInventory` subtracts the pending lock from the displayed inventory). Clicking the glowing lock again de-selects it. Clicking the previewed lock once **selects** it (`.selected-placed`); a following click on another empty cell **moves** it there, and a second click on the lock itself **returns it to the box** — so a double-click on the placed lock puts it back. Clicking an empty cell with nothing selected does nothing (the old "quick-place the smallest lock" branch and its 500 ms suppression window were removed).
